    ...
    const columns: ColumnDef<NoticeData>[] = useMemo(
        () => [
            ...
            // },
            {
                ...
                id: 'actions',
                cell: ({ row }) => {
                    const { iseditable, is_requested, created_by, id } =
                        row.original
                    const noticeData = row.original as any
                    const status = noticeData.status?.toLowerCase() || ''

                    // Check branch_status - only show actions if branch_status is 'active'
                    const branchStatus =
                        noticeData.branch_status ||
                        noticeData.Branch?.status ||
                        noticeData.branch?.status
                            ...
                            {(iseditable === false || is_requested) && (
                                <Tooltip title="Request to Admin">
                                    <Button
                                        size="sm"
                                        onClick={() =>
                                            handleRequestToAdmin(id, 'notice')
                                        } // Pass the correct ID and type
                                        icon={<FaUserShield />}
        ...
        <div className="relative">
            ...
            >
                <div className="p-2">
                    ...
                    <p className="mb-6">
                        Are you sure you want to delete this notice entry?
                    </p>
