...
//             setLoading(false)
//             showNotification('error', 'Failed to load ESI details')
//         }
//     }
...
//     //         showNotification('error', 'Only PDF, ZIP, JPEG, JPG PNG, and GIF files are allowed')
//     //         return
//     //     }
...
//     //         showNotification('error', 'Failed to process certificate')
//     //     }
//     // }
...
//             const normalizedFilename = cleanFilename.replace(/[^a-zA-Z0-9-]/g, "_");
...
//                 certificate: {
//                     data: base64String,
//                     filename: normalizedFilename, // No extension
//                     mimetype: file.type
//                 }
//             }));
...
//             showNotification('error', error.message || 'File processing failed');
//         }
//     };
...
//         if (formData.certificate && typeof formData.certificate !== 'string') {
//             const blob = new Blob(
//                 [Buffer.from(formData.certificate.data, 'base64')], 
//                 { type: formData.certificate.mimetype }
//             );
...
//         // For existing S3 documents
//         if (formData.certificate && typeof formData.certificate === 'string') {
//             // Construct the S3 URL
//             const s3Url = `https://${import.meta.env.VITE_S3_BUCKET}.s3.${import.meta.env.VITE_S3_REGION}.amazonaws.com/${formData.certificate}`;
...
//             if (!id) {
//                 showNotification('error', 'ESI Setup ID is missing')
//                 return
//             }
...
interface ESISetupData {
    ...
    esi_user: string
    password: string
    certificate?: string | CertificateData | null;
    certificateData?: S3CertificateData;
    email: string
    mobile_number: string
    CompanyGroup?: {
        id: number
...
const ESIEditedData: React.FC<ESIEditedDataProps> = ({
    ...
    const fetchESIData = async () => {
        try {
            setLoading(true);
            const response = await dispatch(fetchEsiSetupById(id)).unwrap();
            
            setFormData({
                ...response,
                // Keep certificate as string key for existing files
                certificate: response.certificateData?.key || null
            });
            
            // Set file info if certificate exists
            if (response.certificateData) {
                setFileInfo({
                    name: response.certificateData.filename,
                    type: response.certificateData.mimetype,
    ...
    const handleDocumentView = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        // For new uploads (not yet saved to S3)
        if (formData.certificate && typeof formData.certificate === 'object' && 'data' in formData.certificate) {
            const certData = formData.certificate as CertificateData;
            try {
                const byteCharacters = atob(certData.data);
                const byteNumbers = new Array(byteCharacters.length);
                for (let i = 0; i < byteCharacters.length; i++) {
                    byteNumbers[i] = byteCharacters.charCodeAt(i);
                }
                const byteArray = new Uint8Array(byteNumbers);
                const blob = new Blob([byteArray], { type: certData.mimetype });
                
                // Create download link
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = certData.filename || 'document';
        ...
        if (formData.certificateData) {
            // Create a direct download link to the S3 object
            const downloadUrl = `https://${import.meta.env.VITE_S3_BUCKET}.s3.${import.meta.env.VITE_S3_REGION}.amazonaws.com/${encodeURIComponent(formData.certificateData.key)}`;
            
            // Trigger download
            const a = document.createElement('a');
            a.href = downloadUrl;
            a.download = formData.certificateData.filename || 'document';
            a.target = '_blank';
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
    ...
    const handleSubmit = async () => {
        try {
            setLoader(true)
            // Mark all fields as touched
            let certificateSubmission: any = undefined;
        
            // For new uploads
            if (formData.certificate && typeof formData.certificate === 'object' && 'data' in formData.certificate) {
                certificateSubmission = {
                    data: formData.certificate.data,
                    filename: formData.certificate.filename,
                    mimetype: formData.certificate.mimetype
                };
            }
            // For existing certificates (keep the S3 key)
            else if (typeof formData.certificate === 'string') {
                certificateSubmission = formData.certificate;
            }
            const updateData = {
                group_id: formData.group_id,
                company_id: formData.company_id,
