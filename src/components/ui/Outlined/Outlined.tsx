...
const OutlinedSelect = ({ 
  ...
  isMulti = false, 
  isDisabled = false,
  showClearButton: allowClearButton = true
}: OutlinedSelectProps) => {
  const [selectedValue, setSelectedValue] = useState(value);
  const [hasInput, setHasInput] = useState(false);
  ...
  const handleChange = (newValue: { value: any; }) => {
    if (isMulti) {
      if (selectedValue.some((option: { value: any; }) => option.value === newValue.value)) {
        const updatedValue = selectedValue.filter((option: { value: any; }) => option.value !== newValue.value);
        setSelectedValue(updatedValue);
        onChange(updatedValue);
      } else {
        setSelectedValue([...selectedValue, newValue]);
